...
import { ChevronLeftIcon, ShieldCheck } from '../components/Icons';
...
const ReviewQueuePage: React.FC<ReviewQueuePageProps> = ({ currentUser, onBack, onNavigate, showToast }) => {
                ...
                {showFeedbackInput ? (
                    <div className="animate-slide-up space-y-4">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <button
                                ...
                                className="flex-[2] bg-blue-500 hover:bg-blue-600 text-white py-3 rounded-xl font-bold shadow-lg shadow-blue-500/30 transition-all disabled:opacity-50"
                            >
                                Submit Decision
                            </button>
                        </div>
                    ...
                    <div className="flex gap-2 h-16">
                        <button
                            ...
                            className="flex-1 bg-red-50 hover:bg-red-100 text-red-600 rounded-xl font-bold text-sm transition-all border border-red-200"
                        >
                            Reject
                        </button>
                        <button
                            onClick={() => { setShowFeedbackInput(true); setCurrentAction('REQUEST_CHANGES'); }}
                            className="flex-1 bg-yellow-50 hover:bg-yellow-100 text-yellow-600 rounded-xl font-bold text-sm transition-all border border-yellow-200"
                        >
                            Request Edit
                        </button>
                        <button
                            onClick={() => handleAction('APPROVE')}
                            className="flex-1 bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold text-sm shadow-md transition-all flex items-center justify-center"
                        >
                            Approve
                        </button>
                    </div>
